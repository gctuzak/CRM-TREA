...
async function restoreCleanBackup() {
  const container = await findMySQLContainer();
  if (!container) return false;

  const cleanBackupFile = './mydatabase_backup_clean.sql';
  
  if (!fs.existsSync(cleanBackupFile)) {
    console.error(`❌ ${cleanBackupFile} dosyası bulunamadı!`);
    return false;
  }

  try {
    console.log('🔄 Temiz backup dosyası yükleniyor...');
    ...
    execSync(`docker cp "${cleanBackupFile}" ${container}:/tmp/clean_backup.sql`);
    
    // Veritabanını geri yükle
    console.log('🔄 Veritabanı geri yükleniyor...');
    execSync(`docker exec ${container} bash -c "mysql -u${DB_USER} -p${DB_PASS} --default-character-set=utf8mb4 ${DB_NAME} < /tmp/clean_backup.sql"`);
    
    // Geçici dosyayı temizle
    execSync(`docker exec ${container} rm /tmp/clean_backup.sql`);
    
    console.log('✅ Temiz backup başarıyla yüklendi');
    return true;
  } catch (error) {
...
async function main() {
  console.log('🚀 Temiz Backup Geri Yükleme\n');
  
  const success = await restoreCleanBackup();
  if (success) {
    console.log('\n🧪 Veritabanı test ediliyor...');
    await testDatabase();
    console.log('\n🎉 İşlem tamamlandı! Binlerce kayıt başarıyla yüklendi.');
  }
}
